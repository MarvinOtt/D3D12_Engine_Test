...
{
	acs->ACS_TOP_index = ACS_OBJ_buffers.size();
...
bool DXR_ACS_TOP::Add_ACS_OBJ_multiple(DXR_ACS_OBJ** acs, int size)
{
	...
	{
		acs[i]->ACS_TOP_index = ACS_OBJ_buffers.size() + i;
	}
	int oldsize = ACS_OBJ_buffers.size();
...
bool DXR_ACS_TOP::Build(GraphicsDevice* device)
{
	if (HasBeenBuilt)
	{
		pScratch->buffer->Release();
		pResult->buffer->Release();
		pInstanceDesc->buffer->Release();
	}
