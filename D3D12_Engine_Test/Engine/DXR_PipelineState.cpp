...
bool DXR_PipelineState::Build(GraphicsDevice* device)
{
    int subobject_count = 1 + hitGroups.size() + shaderRS_Pairs.size() * 2;
    ...
    DxilLibrary dxilLib(pDxilLib, shaderNames.data(), shaderNames.size());
    ...
    if (shaderNames.size() > shaderRS_Pairs.size())
    {
        ...
        ExportAssociation* rootAssociation = new ExportAssociation(remainingShaders.data(), remainingShaders.size(), &(subobjects[rootIndex]));
        subobjects[index++] = rootAssociation->subobject;
    }
    ...
    ExportAssociation configAssociation(shaderNames.data(), shaderNames.size(), &(subobjects[shaderConfigIndex]));
