...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	mainWindow->Create(FullScreen, L"DXR_Test", Width, Height, NULL, 2560 * 0);
...
bool InitD3D()
{
	...
	transformation[0] = glm::translate(vec3(-2, 100, 1));
	transformation[1] = glm::translate(vec3(2, 0.1f + 0.0f, 0)) * glm::eulerAngleX(0.3f);

	ACS_BOT_cube = new DXR_ACS_BOT(graphicsDevice, vertexBuffer, indexBuffer);

	ACS_OBJ_plane = new DXR_ACS_OBJ(graphicsDevice, vertexBuffer2, glm::identity<mat4>(), true);
	for (int i = 0; i < 2; ++i)
	{
		if (i == 0)
			ACS_OBJ_cubes.push_back(new DXR_ACS_OBJ(graphicsDevice, ACS_BOT_cube, glm::translate(vec3(0, 9.0f, 0)), true));
		else
	...
	for (int x = 0; x < 10; ++x)
	{
		for (int y = 0; y < 10; ++y)
		{
			ACS_OBJ_cubes.push_back(new DXR_ACS_OBJ(graphicsDevice, ACS_BOT_cube, glm::translate(vec3(x * 1.1f + 3, 0.0f, y * 1.1f)), true));
		}
	}
	...
	{
		ACS_OBJ_house[i] = new DXR_ACS_OBJ(graphicsDevice, curvertexbuffer[i], curindexbuffer[i], glm::identity<mat4>(), true);
	}
	...
	DXR_COL_pipelineState->SetShaders(L"Shaders\\DXR_Shader.hlsl", shaders);
	...
	DXR_COL_pipelineState->Build(graphicsDevice, sizeof(float) * 2, sizeof(float) * 7);
	...
	DXR_AO_pipelineState->SetShaders(L"Shaders\\DXR_Shader_AO.hlsl", shaders2);
	...
	DXR_AO_pipelineState->Build(graphicsDevice, sizeof(float) * 2, sizeof(float) * 1);
	...
	temporalSamplingShader.Load(L"Shaders\\TemporalSampling.hlsl", "main", "ps_5_0");
	Shader basic_VS;
	basic_VS.Load(L"Shaders\\VertexShader.hlsl", "main", "vs_5_0");
	...
	BSV_shader.Load(L"Shaders\\BilateralFilter_vertical.hlsl", "main", "ps_5_0");
	...
	//final
	Shader BS_shader;
	BS_shader.Load(L"Shaders\\BilateralFilter.hlsl", "main", "ps_5_0");
...
void Update()
{
	...
	mat4 newrotmatrix2 = glm::translate(vec3(-1, 0, 0));
	ACS_OBJ_cubes[1]->ChangeTrans(newrotmatrix);
	//ACS_OBJ_cubes[0]->ChangeTrans(newrotmatrix2);
	//ACS_OBJ_cubes[0]->ChangeTrans(ACS_OBJ_cubes[0]->trans);
...
void Render()
{
	...
	d3d_call(graphicsDevice->swapChain->swapChain->Present(1, 0)); // Present current frame
}
